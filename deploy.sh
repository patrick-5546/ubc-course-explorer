...
ssh -o StrictHostKeyChecking=no ubuntu@$1 << 'ENDSSH'
  ...
  docker-compose -f docker-compose.staging.yml exec -T web python manage.py migrate --noinput ; docker-compose -f docker-compose.staging.yml exec -T web python manage.py collectstatic --noinput --clear
ENDSSH
